...
Struct.new("Challenger", :formula, :proc)
...
challengers = [Struct::Challenger.new(base, challenge_formula(base))]
...
1000.times do |i|
  ...
  challengers.each do |chal|
    challengers += 1.times.map {
       ...
       Struct::Challenger.new(mutant, challenge_formula(mutant))                         }
  end
  
  #set = 1000.times.map { rand(10000) }
  #model_result = measure(model, set)
  
  selection = select_challengers(challengers, model_result, set, 100)
